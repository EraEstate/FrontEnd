...
export const propertyCollectionAPI = {
  getMyCollections: async (): Promise<PropertyCollection[]> => {
    const response = await api.get<PropertyCollection[]>('/api/property-collections/my-collections');
    return response.data;
  },

  getCollectionDetails: async (id: string): Promise<PropertyCollection> => {
    const response = await api.get<PropertyCollection>(`/api/property-collections/${id}`);
    return response.data;
  },

  createCollection: async (name: string, description?: string, isPublic?: boolean): Promise<PropertyCollection> => {
    const response = await api.post<PropertyCollection>('/api/property-collections', {
      name,
      description,
      isPublic
    });
  ...
  updateCollection: async (id: string, name?: string, description?: string, isPublic?: boolean): Promise<PropertyCollection> => {
    const response = await api.put<PropertyCollection>(`/api/property-collections/${id}`, {
      name,
      description,
      isPublic
    });
  ...
  deleteCollection: async (id: string): Promise<{ success: boolean; message: string }> => {
    const response = await api.delete<{ success: boolean; message: string }>(`/api/property-collections/${id}`);
    return response.data;
  },

  addItemToCollection: async (collectionId: string, propertyId: string): Promise<PropertyCollectionItem> => {
    const response = await api.post<PropertyCollectionItem>(`/api/property-collections/${collectionId}/items`, {
      propertyId
    });
    return response.data;
  },
  ...
  removeItemFromCollection: async (collectionId: string, propertyId: string): Promise<{ success: boolean; message: string }> => {
    const response = await api.delete<{ success: boolean; message: string }>(`/api/property-collections/${collectionId}/items/${propertyId}`);
    return response.data;
  },

  getPublicSharedCollection: async (shareToken: string): Promise<PropertyCollection> => {
    const response = await api.get<PropertyCollection>(`/api/property-collections/shared/${shareToken}`);
    return response.data;
  }
};
