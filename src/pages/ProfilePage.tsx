    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-x-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-gray-900 flex items-center gap-2">
                ...
                {kycVerified ? (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-xs font-semibold" title={t('profile.kycVerifiedTitle')}>
                    <CheckCircle2 className="w-3 h-3" /> {t('profile.verified')}
                  </span>
                ) : (
                  <Link to="/kyc-verify" className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-xs font-semibold hover:bg-amber-200 transition-colors" title={t('profile.kycUnverifiedTitle')}>
                    <Shield className="w-3 h-3" /> {t('profile.unverified')}
                  </Link>
                )}
              </h2>
              <div className="flex items-center text-gray-600 mt-1">
                <Award className="w-4 h-4 mr-1" />
                <span suppressHydrationWarning>{t('profile.memberSince')} {new Date(userProfile?.createdAt || Date.now()).getFullYear()}</span>
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              >
                ...
                <div className="text-sm font-semibold text-red-800">{t('profile.propertyStats')}</div>
              </Link>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
          ...
          {t('profile.kycIdentity')}
        </h3>
        {kycVerified ? (
          <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 rounded-lg px-3 py-2">
            <CheckCircle2 className="w-4 h-4" />
            <span>{t('profile.kycVerifiedDesc')}</span>
          </div>
        ) : kycStatus === 'PENDING' ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-sm text-red-700 bg-red-50 rounded-lg px-3 py-2">
              ...
              <span>{t('profile.kycRejected')}</span>
            </div>
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">{t('profile.kycRequirement')}</p>
      ...
      {userProfile?.id && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <BadgeDisplay userId={userProfile.id} title={t('profile.yourBadges')} />
          <div className="mt-3">
      ...
      {/* Recent Activity */}
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h3 className="text-xl font-semibold text-gray-900 mb-4">{t('profile.recentActivity')}</h3>
...
const PropertiesTab: React.FC<PropertiesTabProps> = ({
  ...
  <div className="space-y-6">
    <div className="flex justify-between items-center">
      <h3 className="text-xl font-semibold text-gray-900">{t('profile.myProperties')} ({myProperties.length})</h3>
      <Link
        to="/post-property"
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <div className="flex items-center gap-x-4 text-sm text-gray-600">
                  ...
                  <span className="text-sm text-gray-500">
                    <span suppressHydrationWarning>{t('profile.postedOn', { date: new Date(property.createdAt).toLocaleDateString('vi-VN') })}</span>
                  </span>
                </div>
...
const FavoritesTab: React.FC<FavoritesTabProps> = ({
  ...
  <div className="space-y-6">
    <h3 className="text-xl font-semibold text-gray-900">{t('profile.myFavorites')} ({favorites.length})</h3>
        ...
        {favorites.map((favorite: any) => (
          <div key={favorite.id} className="bg-white rounded-lg shadow-sm border p-4">
            ...
            <div className="pt-4 border-t">
              <span className="text-sm text-gray-500">
                <span suppressHydrationWarning>{t('profile.savedOn', { date: new Date(favorite.createdAt).toLocaleDateString('vi-VN') })}</span>
              </span>
            </div>
          </div>
        ))}
...
const SubscriptionTab: React.FC<SubscriptionTabProps> = ({ t }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h3 className="text-xl font-semibold text-gray-900 mb-4">{t('profile.subscriptionHistory')}</h3>
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        ...
        toast.error(t('profile.fetchProfileError'));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-gray-900 mb-4">{t('profile.pleaseLogin')}</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-8">
        {/* {t('profile.inactiveAccountBanner')} */}
